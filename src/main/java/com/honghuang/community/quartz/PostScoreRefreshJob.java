...
public class PostScoreRefreshJob implements Job, CommunityConstant {
    ...
    private void refresh(int postId){
        ...
        //是否精华
        boolean elite = post.getStatus() == 1;
        //评论数量
        ...
        //计算权重
        double w = (elite ? 75 : 0) + commentCount * 10 + likeCount * 2;
        //分数 = 权重(为防止出现负数分数,给定在计算分数公式中w最小值为1)
        double score = Math.log10(Math.max(w,1)) + (post.getCreateTime().getTime() - SCORE_ITEM) / (1000 * 3600 * 24);
